...
2. Display - 480 x 320 pixel TFT LCD with ILI9488 controller, FT6236 capacitive touchscreen and PWM-based backlight control.  SPI-to-Parallel converter allows operation of Display VSPI peripheral at up to the maximum 80 MHz.
...
### Power Control
gCore generates two internal 3.3V rails.  One always on that powers the EFM8 co-processor and one switched by the EFM8 that powers the rest of gCore and user application circuitry.  gCore also makes a switched rail from the Charge Controller (raw battery voltage) available.  Total current from both switched 3.3V and switched raw battery, used by both gCore and user circuitry, should be approximately 1A or less.

gCore can be powered on by multiple sources.

1. Power Button Press - Pressing the button for a configurable time (20 mSec - 2.5 seconds) always switches power on.
2. RTC Alarm - An alarm value can be set in the EFM8 RTC that will power on gCore if it off when the Alarm expires.
3. Charge Start and/or Charge End - The EFM8 can be configured to power on when charging starts or completes (full battery).

It can also be powered off by multiple sources.

1. ESP32 initiated - Code running on the ESP32 can switch power off using the EFM8.  Short Power Button Presses can be detected by code for soft power off.
2. Long Power Button Press - A long Power Button press (~5 seconds) will switch power off regardless of code running on the ESP32.
...
### Display
#### LCD Controller
The ILI9488 LCD controller is configured with an 8-bit parallel interface.  A serial-to-parallel converter, shown in ```gCore_serial_to_parallel.png```, connects the LCD controller to the ESP32 dedicated VSPI peripheral.  This allows driving the LCD controller at the maximum SPI clock rate of 80 MHz.  There are some specific timing requirements, described below, that must be followed.
...
Primarily the CSN signal must be de-asserted after the final SCLK rising edge.  This is handled automatically by the Arduino gCore LCD driver and the ported eSPI_TFT libraries but must be specified as shown when configuring the VSPI peripheral directly in the IDF.
...
3. Pulsing - USB supplying power.  Battery charging.
...
### Micro-SD
The Micro-SD socket is configured to use the ESP32 4-bit data mode interface.  It may also use the 1-bit data mode or as a SPI device (using the ESP32 HSPI peripheral).  The GPIO12-15 signals double as a JTAG debugging port when no Micro-SD Card is installed.
...
Registers are addressed using a 16-bit address.
...
| Register | Address | Size | Type | Description |
| --- | ---| --- | --- | --- |
...
| 2 | Power Button current pressed bit |
...
##### Shutdown Register
The Shutdown register is used to switch power off.  Writing the value 0x0F to this register turns off the switched 3.3V and switched raw battery supply rails.  Writing any other value has no effect.  Register read values are undefined.
...
When setting the time be sure to write all four bytes to the register.  The time will be updated upon completion of the fourth (LSB) byte.

##### Alarm Register
The Alarm register contains the a time that can be used to power on gCore if the associated bit is set in the Wake Control register.  An alarm occurs when the Time register becomes equal to or greater than the Alarm register.
...
The Time Correct register is used to improve the accuracy of the EFM8 RTC.  It may be written with a signed value specifying a number of seconds between time corrections.  Typically it would be written by software observing the accuracy of the RTC over a long period of time.
